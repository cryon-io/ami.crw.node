    ...
    commands = {
        info = {
            ...
            action = function(_options, command, args, cli)
                ...
                local _ok, _infoLua = pcall(loadfile, "__crw/info.lua")
                if not _ok then
                    ami_error(_ok, "Failed to get info of CRW NODE - " .. _infoLua, EXIT_APP_INFO_ERROR)
                end
                local _ok, _error = pcall(_infoLua, OUTPUT_FORMAT == "json")
                ami_assert(_ok, "Failed to get info of CRW NODE - " .. (_error or ""), EXIT_APP_INFO_ERROR)
            end
        },
        setup = {
            options = {
            ...
            action = function(options, command, args, cli)
                ...
                if _noOptions or options.app then
                    ...
                    ami_assert(_ok, "Failed to download crown binaries - " .. (_error or ""), EXIT_SETUP_ERROR)
                end
        ...
        start = {
            ...
            action = function(options, command, args, cli)
                ...
                ami_assert(_ok, "Failed to start CRW NODE - " .. (_error or ""), EXIT_APP_START_ERROR)
            end
        },
        stop = {
            description = "ami 'stop' sub command",
            summary = "Stops the CRW node",
            action = function(options, command, args, cli)
                if options.help then
                    show_cli_help(cli)
                    return
                ...
                ami_assert(_ok, "Failed to stop CRW NODE - " .. (_error or ""), EXIT_APP_START_ERROR)
            end
        },
        validate = {
            ...
            action = function(_options, command, args, cli)
                ...
                -- //TODO: add switches
                ami_assert(eliProc.EPROC, "crown node AMI requires extra api - eli.proc.extra", EXIT_MISSING_API)
                ami_assert(eliFs.EFS, "crown node AMI requires extra api - eli.fs.extra", EXIT_MISSING_API)
                ...
                ami_assert(
                    type(APP.configuration.NODE_TYPE) == "string",
                ...
                log_success("Crown masternode configuration validated.")
            end
        },
        about = {
            ...
            action = function(_options, command, args, cli)
                ...
                local _ok, _aboutFile = eliFs.safe_read_file("__crw/about.hjson")
        ...
        removedb = {
            ...
            action = function(_options, command, args, cli)
                ...
                log_success("Succesfully removed crownd database.")
            end
        },
        remove = {
            index = 7,
